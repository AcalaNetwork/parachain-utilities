...
function BlockTime(): React.ReactElement {
  ...
  const selectedNetworks = config.networks.filter((auxNetwork) => auxNetwork.enabled)
  ...
  const [defaultBlockTimes, setDefaultBlockTimes] = useState<(number | undefined)[]>(
  ...
  const loadExpectedBlockTime = async () => {
    try {
      ...
      const newDefaults = apis.map((auxApi) => getExpectedBlockTime(auxApi))

      formBlocks.setFieldsValue({
        blockTimes: newDefaults.map((auxDefault, index) => formBlocks.getFieldValue('blockTimes')[index] || auxDefault),
      })
  ...
  const checkOptionalFields = (changedValues: Record<string, unknown>, values: BlockTimeFormValues) => {
  ...
  const estimateForBlockNumber = async (blockNumber: number, expectedBlockTimes: number[]) => {
    setIsLoading(true)
    ...
    while (index < selectedNetworks.length) {
      try {
        ...
        let formattedResult = ''
        let type = ''
        const blocksDifference = blockNumber - currentBlockNumber
  ...
  const estimateForDateTime = async (datetime: Moment, expectedBlockTimes: number[]) => {
    setIsLoading(true)
    ...
    while (index < selectedNetworks.length) {
      try {
        ...
        if (inputTimestamp > currentTime) {
          result = currentBlockNumber + Math.ceil((inputTimestamp - currentTime) / expectedBlockTimes[index])
          type = 'Future'
        ...
        setResults((oldResults) => {
          return [
            ...oldResults,
            {
              ...
              type,
              blocksDifference: result - currentBlockNumber,
            },
          ]
        })
  ...
  const searchPastBlockNumber = async (
    ...
    while (leftBlockNumber < rightBlockNumber) {
      ...
      if (directionLeftToRight) {
        ...
        // Estimate using expectedBlockTime
        searchBlockNumber =
          rightBlockNumber - Math.max(1, Math.round((rightBlockTime - targetTime) / expectedBlockTime))
  ...
  const renderBlocksDifference = (row: BlockTimeResult) => {
    return <div>{row.blocksDifference} blocks</div>
  }
  ...
  const columns = [
    ...
    {
      title: 'Blocks Difference',
      key: 'action',
      render: renderBlocksDifference,
    },
  ]
